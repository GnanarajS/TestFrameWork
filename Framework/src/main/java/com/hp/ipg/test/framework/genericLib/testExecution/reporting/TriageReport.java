...
import com.hp.ipg.test.framework.genericLib.testExecution.utils.JenkinsUtils;
...
/**
 * This class generates a triage report html page after every run.
 */
public class TriageReport implements IReporter, ISuiteListener {

    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(TriageReport.class);
    public static final String SAUCE_SESSION_ID = "SauceSessionID";
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
        ...
        if ((reportName.equals("MobileApp")) || (reportName.equals("ApiTests")) || (reportName.equals("WorkFlowTests")) || (reportName.equals("WebAppTest"))) {
            reportName = "";
        }

        this.testSuites = suites;
        testOutputDirectory = BuildUtils.getTestOutputDirectory();
        File report = new File(testOutputDirectory, reportName + (!reportName.equals("") ? "-" : "") + "triageReport.html");
        if (report.exists()) {
    ...
    private void generateDocumentHead() throws ParseException {
        ...
        htmlBuilder.append(String.format("<h1>Triage Report [%s]</h1>", formatter.format(today)));
    }
    ...
    private void generateFailureTable(IResultMap map, String tableTitle, String cssClass) throws InvalidKeyException, NoSuchAlgorithmException {
        ...
        for (ITestResult result : getSortedMethods(map, new FailedTestSorter<ITestResult>())) {
            ...
            if (result.getMethod().getMethodName().equals(testMethod.getMethodName())) {
                ...
                if (reportName.equals("MobileApp") && System.getProperty("sauce.enable").equals("true")) {
                    try {
                        htmlBuilder.append(String.format("<td>%s</td>",
                            "<a href=" + result.getAttribute("testResults") + " target=blank>sauceLabs</a>"
                            ));
                    }
                    catch(NullPointerException e){}
                }
                else if(reportName.equals("WebAppTest") && System.getProperty("sauce.enable").equals("true")) {
                    try {
                        htmlBuilder.append(String.format("<td>%s</td>",
                            getPublicTestLink(result.getAttribute("sauceJobId").toString())));
                    }
                    catch(NullPointerException e){}
                }
    ...
    /**
     * Giving a jobId and SauceLabs credentials, this returns a public link to the test that doesn't require login.
     *
     * @param jobId
     *            The test's session id obtained from RemoteWebDriver
     * @return public link to triage results that doesn't require login.
     */
    public String getPublicTestLink(String jobId) throws NoSuchAlgorithmException, InvalidKeyException {
        String result = "";
        if (jobId != null) {
            String key = WebSauceClient.getUser() + ":" + WebSauceClient.getSauceApiKey();

            SecretKeySpec keySpec = new SecretKeySpec(key.getBytes(), "HmacMD5");
            Mac mac = Mac.getInstance("HmacMD5");
            mac.init(keySpec);

            byte[] rawHmac = mac.doFinal(jobId.getBytes());
            String authToken = Hex.encodeHexString(rawHmac);
            result = String.format("<a href=\"https://saucelabs.com/jobs/%s?auth=%s\" target=\"_blank\">sauceLabs</a>",
                    jobId, authToken);
        }
        return result;
    }
    ...
    private void generateSeleniumTable(ITestContext testContext) throws IOException {
        ...
        if((reportName.equals("MobileApp"))) {
            platformOS = System.getProperty("appium.platformName");
            deviceName = System.getProperty("appium.deviceName");
        }
        else if((reportName.equals("WebAppTest"))) {
            platformOS = System.getProperty("selenium.platformName");
            deviceName = System.getProperty("selenium.browserName");
        }
        String stack = System.getProperty("spring.profiles.active");

        LOGGER.info("ACTIVE PROF: " + stack);

        if (JenkinsUtils.isRunningOnJenkins()) {
            buildLink = String.format("<a href=\"%s/\">Build #%s</a>",
                    JenkinsUtils.getJenkinsBuildUrl().toString(),
                    JenkinsUtils.getJenkinsBuildNumber());
        }

        htmlBuilder.append("<div><table>");

        htmlBuilder.append("<tr>");
        htmlBuilder.append(String.format("<th class=summaryTable>%s</th><th class=summaryTable>%s</th><th class=summaryTable>%s</th><th class=summaryTable>%s</th><th class=summaryTable>%s</th>",
                "Build",
                "Platform OS",
                "Device Name",
                "Stack",
        ...
        htmlBuilder.append(String.format("<tr><td align=center>%s</td><td align=center>%s</td><td align=center>%s</td><td align=center>%s</td><td align=center>%s</td></tr>",
                buildLink,
                platformOS,
                deviceName,
                stack,
    ...
    /**
     * Creates html link to the matching snapshot if found.
     */
    private String getSnapshotLink(ITestNGMethod method, ITestResult result) {
        ITestContext context = result.getTestContext();
        String browserDirPrefix = "browser";
        String desktopDirPrefix = "desktop";
        String fileName = String.format("%s.%s_%s.png",
                method.getTestClass().getName(),
                method.getMethodName(),
                result.getAttribute("TestInstanceId"));
        File snapshot = new File(testOutputDirectory, String.format("%s/%sSnapshots/%s", context.getSuite().getName(), browserDirPrefix, fileName));
        ...
        if (snapshot.exists()) {
            ...
            snapshot = new File(testOutputDirectory, String.format("%s/%sSnapshots/%s", context.getSuite().getName(), desktopDirPrefix, fileName));
            if (snapshot.exists()) {
                linkTitle = "desktop";
            }
        }
        ...
        if (!linkTitle.isEmpty()) {
            snapshotLink = String.format("<a href=\"./%s/%s/%s\">%s</a>", context.getSuite().getName(), linkTitle + "Snapshots", snapshot.getName(), linkTitle);
        }
    ...
    /**
     * Creates html link to the matching browser logs if found.
     */
    private String getBrowserLogLink(ITestNGMethod method, ITestResult result) {
        ITestContext context = result.getTestContext();
        String fileName = String.format("%s.%s_%s.log",
                method.getTestClass().getName(),
                method.getMethodName(),
                result.getAttribute("TestInstanceId"));
        File log = new File(testOutputDirectory, String.format("%s/browserLogs/%s", context.getSuite().getName(), fileName));

        String browserLogLink = "";

        if (log.exists()) {
            browserLogLink = String.format("<a href=\"./%s/%s/%s\">%s</a>", context.getSuite().getName(), "browserLogs", log.getName(), "console-log");
        }
